...
import dotenv
import torch
from transformers import LlamaTokenizer, LlamaForCausalLM  # 변경됨
...
class OpenLlamaGenerator:
    """
    OpenLlama 계열 모델 로드 + 싱글/배치 텍스트 생성 유틸.
    """
    def __init__(
        self,
        ...
        # LlamaForCausalLM 직접 사용 (공식 권장)
        self.model = LlamaForCausalLM.from_pretrained(
            model_name,
            cache_dir=cache_dir,
            torch_dtype=torch_dtype,
            device_map=device_map,  # auto로 설정하면 자동 배치
            **model_kwargs,
        )

        # pad_token이 없으면 eos로 세팅
